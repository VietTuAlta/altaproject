...
const DeviceList = React.memo((props: DeviceListProps) => {
  ...
  const handleInputChange = (e:any, columnProp:number) => {
    ...
    debouncedSearchNumber(filter1Value,'2024-01-01', '2025-12-31', 'All', searchText, filter2Value)
  };
  ...
  const handleFilter1Change = async (value:string) => {
    switch(props.columns){
      ...
      case 4:
        ...
        break;
      default:
        await handleSearchNumber(value,'2024-01-01', '2025-12-31', 'All', '', 'All');
        setFilter1Value(value);
        break;
    }
  }
  const handleFilter2Change = async (value:string) => {
    switch(props.columns){
      case 1:
        setDisplayData(filter1Value=='All'?value!='All'?internalData.filter((x:any)=>x.connected==value)
        :internalData
      :value!='All'?internalData.filter((x:any)=>x.connected==value && x.operationStatus==filter1Value)
      ...
      case 4:
          ...
          break;
      default:
        await handleSearchNumber(filter1Value,'2024-01-01', '2025-12-31','All', '', value);
        setFilter2Value(value);
        break;
    }
  }
  ...
  const handleSearchNumber = async (filter1Value:string, start:string, end:string, filter2Value:string, searchValue:string, status:string) => {
    ...
    try {
      const [totalRecords, tempData] = await Promise.all([
        getTotalNumber(filter1Value,'2024-01-01','2025-12-31', filter2Value, searchParam, status),
        getProvidedNumber(filter1Value,'2024-01-01','2025-12-31', filter2Value, searchParam, 1, pageSize, status)]);
        setTotalRecords(totalRecords);
        setDisplayData(tempData);
    } catch (error) {
      console.error("Error fetching data:", error);
  ...
  async function getPN(){
    ...
    if(props.columns==1){
      ...
      setLoading(true);
      const [totalRecords, tempData] = await Promise.all([
      getTotalNumber(filter1Value,'2024-01-01','2025-12-31', 'All', searchText==''?'___':searchText, filter2Value),
      getProvidedNumber(filter1Value,'2024-01-01','2025-12-31', 'All', searchText==''?'___':searchText, currentPage, pageSize, filter2Value)]);
      setLoading(false)
      setTotalRecords(totalRecords);
      setDisplayData(tempData);
    }
  ...
  connection.on("ReceiveProvidedNumber", async(code:string)=>{
      ...
      const [totalRecords, tempData] = await Promise.all([
      getTotalNumber(filter1Value,'2024-01-01','2025-12-31', 'All', searchText==''?'___':searchText, filter2Value),
      getProvidedNumber(filter1Value,'2024-01-01','2025-12-31', 'All', searchText==''?'___':searchText, 1, pageSize, filter2Value)]);
      setLoading(false)
      setTotalRecords(totalRecords);
      setDisplayData(tempData);
  })
